...
def main():
    ...
    ap.add_argument(
        "--save_model_dir", type=str, default=None, help="Directory to save the quantized model."
    )
    args = ap.parse_args()
    ...
    if args.quant_format is not None and not customized_model:
        assert args.calib_parquet is not None, (
            "--calib_parquet is required when quant_format is not None"
        )
        assert 0 < args.num_of_calib_clips <= 5000, (
            "--num_of_calib_clips must be between 1 and 5000"
        )
    ...
    if args.save_model_dir is not None:
        save_dir = os.path.join(
            args.save_model_dir,
            f"alpamayo1.5{'_' + str(args.quant_format) if args.quant_format is not None else '_fp16'}{'_' + str(args.auto_quantize_bits) + 'bits' if args.quant_format == 'auto' else ''}{'_weight_only' if args.quant_weight_only else ''}{'_calib' + str(args.num_of_calib_clips) if args.quant_format is not None else ''}",
        )
