...
_OPTIONAL_REWARD_KEYS: list[str] = [
    "collision_weight",
]
...
def _get_reward_cfg(config: object | None) -> dict[str, float]:
    ...
    cfg = {k: float(reward_cfg[k]) for k in _REQUIRED_REWARD_KEYS}
    for k in _OPTIONAL_REWARD_KEYS:
        if k in reward_cfg:
            cfg[k] = float(reward_cfg[k])
    return cfg
...
def compute_reward(
    ...
    *,
    ...
    Trajectory and comfort match
    :func:`alpamayo1_x_rl.rewards.aggregated_reward.compute_reward`.
    ...
    # collision cost (optional)
    collision_cost = 0.0
    collision_weight = w.get("collision_weight", 0.0)
    collision_by_t: dict[str, float] = {}
    if collision_weight > 0.0 and reference.get("obstacle_bbox_future") is not None:
        from alpamayo1_x_rl.rewards.collision_reward import compute_collision_cost

        collision_cost, collision_by_t = compute_collision_cost(
            reference=reference,
            predicted_fut_xyz=predicted_fut_xyz,
            predicted_fut_rot=predicted_fut_rot,
        )
    ...
    traj_component = -w["traj_l2_weight"] * (l2_dist / ade_threshold)
    comfort_component = w["comfort_weight"] * comfort_score
    collision_component = -collision_weight * collision_cost
    if pred_cot_decoded:
        reasoning_component = w["reasoning_weight"] * (reasoning_score / abs(reasoning_threshold))
    else:
        reasoning_component = -w["reasoning_weight"]

    final_reward = (
        traj_component + comfort_component + reasoning_component + collision_component
    )

    if l2_dist >= 5.0:
        final_reward = -w["traj_l2_weight"] * 2.0

    logger.debug(
        f"[compute_reward] l2={l2_dist:.3f} reasoning={reasoning_score:.3f} "
        f"collision={collision_cost:.3f} cot_decoded={pred_cot_decoded} final={final_reward:.4f}"
    )
    ...
    reward_dict.update(collision_by_t)
