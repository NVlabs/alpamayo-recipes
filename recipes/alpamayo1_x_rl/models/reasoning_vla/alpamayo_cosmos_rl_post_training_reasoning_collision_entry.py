...
_PAI_REASONING_OBSTACLE_LOCAL_DIR = os.getenv("ALPAMAYO_PAI_REASONING_OBSTACLE_LOCAL_DIR")
if not _PAI_REASONING_OBSTACLE_LOCAL_DIR:
    raise RuntimeError(
        "Missing required env var ALPAMAYO_PAI_REASONING_OBSTACLE_LOCAL_DIR "
...
def _reasoning_vla_reward_fn(to_be_evaluated, reference=None, *args, config=None, **kwargs):
    ...
    from alpamayo1_x_rl.rewards.aggregated_reward_with_reasoning_collision import compute_reward
...
REASONING_VLA_SPEC = ModelSpec(
    ...
    hydra_overrides=[
        f"data.train.dataset.local_dir={_PAI_REASONING_OBSTACLE_LOCAL_DIR}",
        ...
        "data.train.dataset.load_obstacles=true",
    ],
)
